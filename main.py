...
optimizer_algorithm = optim.Adam(model_2layer.parameters(), lr = 0.00075)
...
for epoch in range(number_of_epochs):
    for i in range(0, len(features), batch_size):
        ...
        label_prediction = model_2layer(features_batch)
        ...
        with open("model_2layer_training.log", "a") as log:
...
with torch.no_grad():
    output_validation = model_2layer(features)

accuracy = (output_validation.round() == label).float().mean()
print(f"Accuracy {accuracy}")

#make class predictions with the model
predictions = (model_2layer(features) > 0.5).int()
